...
Bool PluginsManager_LoadPlugin( void* plugins, Name pluginName, void* _context ) {
	...
	thePlugin = PluginLoader_NewLocal( pluginName, pluginDirectories );
		...
		Stg_Component_DefaultConstructorFunction* defaultNew ) 
{
	...
	/* Check whether the plugin has already been added to the component register */
	...
	/* Add plugin if it isn't already there. */
	if (functionCheck == NULL) {
		 ...
		 * but not to this PluginsManager instance, then just instantiate another Codelet instance
		 * and add it to this PluginsManager's codelet array. */
		codeletInstance = defaultNew( codeletName );
		result = Stg_ObjectList_Append( self->codelets, codeletInstance );
