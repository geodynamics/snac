...
const Type SnacWinklerG3Force_Type = "SnacWinklerG3Force";
...
Index _SnacWinklerG3Force_Register( PluginsManager* pluginsMgr ) {
	return PluginsManager_Submit( pluginsMgr, 
				      ...
				      "0", 
				      _SnacWinklerG3Force_DefaultNew );
}


void* _SnacWinklerG3Force_DefaultNew( Name name ) {
...
void _SnacWinklerG3Force_Construct( void* component, Stg_ComponentFactory* cf, void* data ) {
	...
	if(Spherical) {
		...
		EntryPoint_InsertBefore(
								...
								"SnacTimeStepZero",
								SnacWinklerG3Force_Type,
								_SnacWinklerG3Force_InitialConditions,
								SnacWinklerG3Force_Type);
		EntryPoint_Append(
				  Context_GetEntryPoint( context, Snac_EP_Force ),
				  SnacWinklerG3Force_Type,
				  _SnacWinklerG3Force_Apply_Spherical,
				  SnacWinklerG3Force_Type );
	}
	else {
		EntryPoint_InsertBefore(
		...
		EntryPoint_Append(
				  Context_GetEntryPoint( context, Snac_EP_Force ),
				  SnacWinklerG3Force_Type,
				  _SnacWinklerG3Force_Apply,
				  SnacWinklerG3Force_Type );
	}
	EntryPoint_Append(
		Context_GetEntryPoint( context, AbstractContext_EP_Sync ),
		"SnacWinklerG3Force_Dump",
		_SnacWinklerG3Force_DumpInitForce,
		SnacWinklerG3Force_Type );
}
