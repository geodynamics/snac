...
void SnacCreepPlastic_Constitutive( void* _context, Element_LocalIndex element_lI, Snac_TetraStressTensor tetraStress ) {
	Snac_Context* context = (Snac_Context*)_context;
	Snac_Element* element = Snac_Element_At( context, element_lI );
	SnacCreepPlastic_Element* creepplasticElement = ExtensionManager_Get( context->mesh->elementExtensionMgr, element, SnacCreepPlastic_ElementHandle );
	...
	Journal_Firewall( (int)(temperatureEP != NULL), context->Snac_Error, 
		  "CreepPlastic plugin requires Temperature plugin. Add SnacTemperature to the plugins list in the input file\n" );

	/* If this is a CreepPlastic material, calculate its stress. */
	if ( material->rheology & Snac_Material_CreepPlastic ) {
		...
		for( tetra_I = 0; tetra_I < Tetrahedra_Count; tetra_I++ ) {
			...
			plasticStrain = creepplasticElement->plasticStrain[tetra_I];
			plasticstrainTensor = &creepplasticElement->plasticstrainTensor[tetra_I];
			creepstrainTensor = &creepplasticElement->creepstrainTensor[tetra_I];
			...
			/* Trial stress by power law creep. */
			{
				...
				beta_new1 = beta_old;
				res1 = straind0 - stressd0/(2.0*rmu) - (*creepstrainTensor)[0][0] - 0.5*context->dt*(beta_old+beta_new);
				residual = res1*res1;

				beta_old2 = beta * stressd1/fabs(s[0]-s[2]);
				beta_new2 = beta_old;
				res2 = straind1 - stressd1/(2.0*rmu) - (*creepstrainTensor)[1][1] - 0.5*context->dt*(beta_old+beta_new);
				residual += res2*res2;

				beta_old3 = beta * stressd2/fabs(s[0]-s[2]);
				beta_new3 = beta_old;
				res3 = straind2 - stressd2/(2.0*rmu) - (*creepstrainTensor)[2][2] - 0.5*context->dt*(beta_old+beta_new);
				residual += res3*res3;
				
				beta_old4 = beta * (*stress)[0][1]/fabs(s[0]-s[2]);
				beta_new4 = beta_old;
				res4 = (*strain)[0][1] - (*stress)[0][1]/(2.0*rmu) - (*creepstrainTensor)[0][1] 
					- 0.5*context->dt*(beta_old+beta_new);
				residual += res4*res4;
			...
			/* Start plastic correction step. */
			{
				...
				if(yielded) {
					...
					creepplasticElement->plasticStrain[tetra_I] += sqrt( 0.5f * ((dep1-depm) * (dep1-depm) + (dep2-depm) * (dep2-depm) + (dep3-depm) * (dep3-depm) + depm*depm) );
				...
				depls += creepplasticElement->plasticStrain[tetra_I]*element->tetra[tetra_I].volume;
				totalVolume += element->tetra[tetra_I].volume;
			}
			/* End of the plastic (brittle) part. */
		}
		/* volume-averaged accumulated plastic strain, aps */
		creepplasticElement->aps = depls/totalVolume;
	}
