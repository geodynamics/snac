...
void _SnacCreepMohrCoulomb_DumpPlasticStrain( void* _context ) {
	...
	if( context->timeStep ==0 || (context->timeStep-1) % context->dumpEvery == 0 ) {
		...
		for( element_lI = 0; element_lI < context->mesh->elementLocalCount; element_lI++ ) {
			...
			SnacCreepMohrCoulomb_Element*			elementExt = ExtensionManager_Get(
...
void _SnacCreepMohrCoulomb_DumpPlasticStrainTensor( void* _context ) {
	Snac_Context*				context = (Snac_Context*) _context;
	SnacCreepMohrCoulomb_Context*			contextExt = ExtensionManager_Get(
							context->extensionMgr,
							context,
...
void _SnacCreepMohrCoulomb_DumpViscosity( void* _context ) {
	Snac_Context*				context = (Snac_Context*) _context;
	SnacCreepMohrCoulomb_Context*		contextExt = ExtensionManager_Get(
								   context->extensionMgr,
								   context,
