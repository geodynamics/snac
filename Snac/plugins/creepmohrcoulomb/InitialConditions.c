...
void SnacCreepMohrCoulomb_InitialConditions( void* _context, void* data ) {
...
// loop over the phase using the dictionary
        ...
        if( materialList ) {
                ...
                while( materialEntry ) {
                        context->materialProperty[PhaseI].rheology |= Snac_Material_CreepMohrCoulomb;
	...
	if( restart ) {
		...
		for( element_lI = 0; element_lI < context->mesh->elementLocalCount; element_lI++ ) {
			...
			for( tetra_I = 0; tetra_I < Tetrahedra_Count; tetra_I++ ) {
				...
				creepplasticElement->plasticStrain[tetra_I] = tetraStrain;
				/* not the actual viscosity at restartStep, but doesn't affect the calculation afterwards */
				creepplasticElement->viscosity[tetra_I] = vis_min;
			}//for
			fscanf( fp2, "%le", &plStrain);
			creepplasticElement->aps = plStrain;
		}// for
		if( fp1 )
			fclose( fp1 );
		if( fp2 )
		...
		for( element_lI = 0; element_lI < context->mesh->elementLocalCount; element_lI++ ) {
			...
			SnacCreepMohrCoulomb_Element*	creepplasticElement = ExtensionManager_Get( context->mesh->elementExtensionMgr, element,
                                                                                                                SnacCreepMohrCoulomb_ElementHandle );
                        double plStrain = creepplasticElement->aps;
                        vis_min = context->materialProperty[element->material_I].vis_min;
			Tetrahedra_Index	tetra_I;
			for( tetra_I = 0; tetra_I < Tetrahedra_Count; tetra_I++ ) {
				creepplasticElement->plasticStrain[tetra_I] = plStrain;
				creepplasticElement->viscosity[tetra_I] = vis_min;
			}//for
		}//for 
}//else
