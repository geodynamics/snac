...
void _SnacPlasticMaxwell_DumpIRheology( void* _context ) {
	...
	if( context->timeStep ==0 || (context->timeStep-1) % context->dumpEvery == 0 ) {
		...
		for( element_lI = 0; element_lI < context->mesh->elementLocalCount; element_lI++ ) {
			...
			int tmpInt = element->irheology;
			if(context->timeStep == 0)
				element->irheology = 0;
			/* Take average of tetra plastic strain for the element */
			fwrite( &tmpInt, sizeof(int), 1, contextExt->irheologyOut );
		}
		fflush( contextExt->irheologyOut );
	}
}
