...
**
** $Id: testUpdateElement-tetra-volume-4-4-4.c 3243 2006-10-12 09:04:00Z SteveQuenette $
**
**~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	for( elementIndex = 0; elementIndex < snacContext->mesh->elementLocalCount; elementIndex++ ) {
		...
		if( elementIndex == 0 ) {
			for( tetraIndex = 0; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
				printf( "Element: 0, Tetrahedra: %u, Volume: %g\n", tetraIndex, Snac_Element_At( snacContext, elementIndex )->tetra[tetraIndex].volume );
			}
		}
		else {
			for( tetraIndex = 0, error = False; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
				if( Snac_Element_At( snacContext, elementIndex )->tetra[tetraIndex].volume != 
					Snac_Element_At( snacContext, 0 )->tetra[tetraIndex].volume )
				{
					printf( "Element: %u, Tetrahedra: %u, Volume: %g\n", elementIndex, tetraIndex, Snac_Element_At( snacContext, elementIndex )->tetra[tetraIndex].volume );
					error = True;
				}
			}
			if( error ) {
