...
**
** Role:
**	Tests Tetrahedra_SurfaceNormal function
**
** $Id: testTetrahedra-face-normal-4-4-4.c 3243 2006-10-12 09:04:00Z SteveQuenette $
**
**~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
int main( int argc, char* argv[] ) {
	...
	Normal normal0[Tetrahedra_Count][Tetrahedra_Surface_Count];
	...
	for( tetraIndex = 0; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
		...
		for( faceIndex = 0; faceIndex < Tetrahedra_Surface_Count; faceIndex++ ) {
			 Tetrahedra_SurfaceNormal( 
				...
				&normal0[tetraIndex][faceIndex] );
			printf( "Element: 0, Tetrahedra: %u, Face: %u, Normal: %g %g %g\n", tetraIndex, faceIndex, 
				normal0[tetraIndex][faceIndex][0], normal0[tetraIndex][faceIndex][1], 
				normal0[tetraIndex][faceIndex][2] );
		}
	...
	for( elementIndex = 1; elementIndex < snacContext->mesh->elementLocalCount; elementIndex++ ) {
		...
		for( tetraIndex = 0, error = False; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
			...
			for( faceIndex = 0; faceIndex < Tetrahedra_Surface_Count; faceIndex++ ) {
				...
				if( normal[0] != normal0[tetraIndex][faceIndex][0] ||
					normal[1] != normal0[tetraIndex][faceIndex][1] ||
					normal[2] != normal0[tetraIndex][faceIndex][2] ) 
				{
					printf( "Element %u: Tetrahedra: %u, Face: %u, Normal: %g %g %g (difference: %g %g %g) \n",
						elementIndex, tetraIndex, faceIndex, normal[0], normal[1], normal[2], 
						normal0[tetraIndex][faceIndex][0] - normal[0],
						normal0[tetraIndex][faceIndex][1] - normal[1],
						normal0[tetraIndex][faceIndex][2] - normal[2] );
					error = True;
				}
			}
		}
