...
**
** $Id: testUpdateElement-tetra-face-area-4-4-4.c 3243 2006-10-12 09:04:00Z SteveQuenette $
**
**~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
		...
		if( elementIndex == 0 ) {
			for( tetraIndex = 0; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
				...
				for( faceIndex = 0; faceIndex < Tetrahedra_Surface_Count; faceIndex++ ) {
					printf( "Element: 0, Tetrahedra: %u, Face: %u, Area: %g\n", tetraIndex, faceIndex, Snac_Element_At( snacContext, elementIndex )->tetra[tetraIndex].surface[faceIndex].area );
				}
			}
		}
		else {
			for( tetraIndex = 0, error = False; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
				...
				for( faceIndex = 0; faceIndex < Tetrahedra_Surface_Count; faceIndex++ ) {
					if( Snac_Element_At( snacContext, elementIndex )->tetra[tetraIndex].surface[faceIndex].area
						!= Snac_Element_At( snacContext, 0 )->tetra[tetraIndex].surface[faceIndex].area )
					{
						printf( "Element: %u, Tetrahedra: %u, Face %u, Area: %g\n", elementIndex, faceIndex, tetraIndex, Snac_Element_At( snacContext, elementIndex )->tetra[tetraIndex].surface[faceIndex].area );
						error = True;
					}
				}
			}
