...
**
** Role:
**	Tests Tetrahedra_Volume function
**
** $Id: testTetrahedra-volume-4-4-4.c 3243 2006-10-12 09:04:00Z SteveQuenette $
**
**~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
int main( int argc, char* argv[] ) {
	...
	Volume volume0[Tetrahedra_Count];
	...
	for( tetraIndex = 0; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
		volume0[tetraIndex] = Tetrahedra_Volume( 
			Snac_Element_NodeCoord( snacContext, 0, TetraToNode[tetraIndex][0] ),
			Snac_Element_NodeCoord( snacContext, 0, TetraToNode[tetraIndex][1] ),
			Snac_Element_NodeCoord( snacContext, 0, TetraToNode[tetraIndex][2] ),
			Snac_Element_NodeCoord( snacContext, 0, TetraToNode[tetraIndex][3] ) );
		printf( "Element: 0, Tetrahedra: %u, Volume: %g\n", tetraIndex, volume0[tetraIndex] );
	}
	...
	for( elementIndex = 1; elementIndex < snacContext->mesh->elementLocalCount; elementIndex++ ) {
		...
		for( tetraIndex = 0, error = False; tetraIndex < Tetrahedra_Count; tetraIndex++ ) {
			...
			volume = Tetrahedra_Volume( 
				Snac_Element_NodeCoord( snacContext, elementIndex, TetraToNode[tetraIndex][0] ),
				Snac_Element_NodeCoord( snacContext, elementIndex, TetraToNode[tetraIndex][1] ),
				Snac_Element_NodeCoord( snacContext, elementIndex, TetraToNode[tetraIndex][2] ),
				Snac_Element_NodeCoord( snacContext, elementIndex, TetraToNode[tetraIndex][3] ) );
			
			if( volume != volume0[tetraIndex] ) {
				printf( "Element %u: Tetrahedra: %u, Volume: %g (difference: %g) \n", elementIndex, tetraIndex, 
					volume, volume0[tetraIndex] - volume );
				error = True;
			}
		}
