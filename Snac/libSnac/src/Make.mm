...
PROJ_SRCS = ${def_srcs}
PROJ_CC_FLAGS += -I$(BLD_INCDIR)/$(PROJECT) -I$(BLD_INCDIR)/StGermain -I$(STGERMAIN_INCDIR)/ -I$(STGERMAIN_INCDIR)/StGermain `xml2-config --cflags` -DCURR_MODULE_NAME=\"${def_lib}\"
PROJ_LIBRARIES = -L$(BLD_LIBDIR) -L$(STGERMAIN_LIBDIR)/ -lStGermainFD -lStGermain -lStGermainDiscretisation -lStGermainBase `xml2-config --libs` $(MPI_LIBPATH) $(MPI_LIBS)
LCCFLAGS =
